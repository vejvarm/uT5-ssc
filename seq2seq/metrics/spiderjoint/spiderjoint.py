...
import evaluate
from seq2seq.metrics.spiderjoint.spider_test_suite import compute_test_suite_metric
from seq2seq.metrics.spiderjoint.spider_exact_match import compute_exact_match_metric
import datasets
...
class SpiderJoint(evaluate.Metric):
    def __init__(
        self,
        ...
        **kwargs
        ...
        self.lang: Optional[str] = kwargs.get("metric_lang", "joint")
        if self.lang not in ["cypher", "sql", "sparql", "joint"]: raise NotImplementedError("Metric langugage must be `sql`, `spider` `cypher` or `joint`")
