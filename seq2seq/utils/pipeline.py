...
class Text2SQLGenerationPipeline(Text2TextGenerationPipeline):
    ...
    def _pre_process(self, input: Text2SQLInput) -> str:
        ...
        serialized_schema = serialize_schema(
            ...
            db_id=input.db_id,
            db_column_names=schema.get("db_column_names", []),
            db_table_names=schema.get("db_table_names", []),
            db_column_types=schema.get("db_column_types", []),
            db_primary_keys=schema.get("db_primary_keys", []),
            db_foreign_keys=schema.get("db_foreign_keys", []),
