...
def load_dataset(
    ...
    _spider_dataset_dict: Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths["spider"], cache_dir=model_args.cache_dir, trust_remote_code=data_args.trust_remote_code
    )
    _spider_metric: Callable[[], Metric] = lambda: evaluate.load(
    ...
    _cosql_dataset_dict: Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths["cosql"], cache_dir=model_args.cache_dir, trust_remote_code=data_args.trust_remote_code
    )
    _cosql_metric: Callable[[], Metric] = lambda: evaluate.load(
    ...
    _spider_realistic_dataset_dict : Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths['spider_realistic'], cache_dir=model_args.cache_dir, trust_remote_code=data_args.trust_remote_code
    )
    _spider_realistic_metric: Callable[[], Metric] = lambda: evaluate.load(
    ...
    _spider_syn_dataset_dict : Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths['spider_syn'], cache_dir=model_args.cache_dir, trust_remote_code=data_args.trust_remote_code
    )
    _spider_syn_metric: Callable[[], Metric] = lambda: evaluate.load(
    ...
    _spider_dk_dataset_dict : Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths['spider_dk'], cache_dir=model_args.cache_dir, trust_remote_code=data_args.trust_remote_code
    )
    _spider_dk_metric: Callable[[], Metric] = lambda: evaluate.load(
        path=data_args.metric_paths["spider_dk"], config_name=data_args.metric_config, test_suite_db_dir=data_args.test_suite_db_dir
    )

    # adding spider ssc dataset, using metrics and schemas from spider as is
    _spider_ssc_dataset_dict : Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths['spider_ssc_sql'], cache_dir=model_args.cache_dir, trust_remote_code=data_args.trust_remote_code
    )
    _spider_ssc_metric: Callable[[], Metric] = lambda: evaluate.load(
        path=data_args.metric_paths["spider_ssc_sql"], config_name=data_args.metric_config, test_suite_db_dir=data_args.test_suite_db_dir
    )    
    ...
    _spider_ssc_cypher_dataset_dict : Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths['spider_ssc_cypher'], cache_dir=model_args.cache_dir, trust_remote_code=data_args.trust_remote_code
    )
    _spider_ssc_cypher_metric: Callable[[], Metric] = lambda: evaluate.load(
        path=data_args.metric_paths["spider_ssc_cypher"], config_name=data_args.metric_config, test_suite_db_dir=data_args.test_suite_db_dir
    )    

    _spider_ssc_joint_dataset_dict : Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths['spider_ssc_joint'], cache_dir=model_args.cache_dir, trust_remote_code=data_args.trust_remote_code
    )
    _spider_ssc_joint_metric: Callable[[], Metric] = lambda: evaluate.load(
        path=data_args.metric_paths["spider_ssc_joint"], config_name=data_args.metric_config, test_suite_db_dir=data_args.test_suite_db_dir
    )    

    _sm3_sql_dataset_dict: Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths['sm3_sql'], cache_dir=model_args.cache_dir, trust_remote_code=data_args.trust_remote_code
    )
    _sm3_sql_metric: Callable[[], Metric] = lambda: evaluate.load(
        path=data_args.metric_paths["sm3_sql"], config_name=data_args.metric_config, test_suite_db_dir=data_args.test_suite_db_dir
    )

    _sm3_sparql_dataset_dict: Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths['sm3_sparql'], cache_dir=model_args.cache_dir, trust_remote_code=data_args.trust_remote_code
    )
    _sm3_sparql_metric: Callable[[], Metric] = lambda: evaluate.load(
        path=data_args.metric_paths["sm3_sparql"], config_name=data_args.metric_config, test_suite_db_dir=data_args.test_suite_db_dir
    )

    _sm3_cypher_dataset_dict: Callable[[], DatasetDict] = lambda: datasets.load.load_dataset(
        path=data_args.dataset_paths['sm3_cypher'], cache_dir=model_args.cache_dir, trust_remote_code=data_args.trust_remote_code
    )
    _sm3_cypher_metric: Callable[[], Metric] = lambda: evaluate.load(
        path=data_args.metric_paths["sm3_cypher"], config_name=data_args.metric_config, test_suite_db_dir=data_args.test_suite_db_dir
    )

    # print(f"\n\n{_spider_ssc_dataset_dict}\n\n")
    ...
    # print(data_args.dataset_paths["spider_ssc_sql"])
    # print(model_args.cache_dir)
    # print(_spider_dataset_dict())
    ...
    elif data_args.dataset == "spider_ssc_sql":
        # spider4ssc_sql
        metric = _spider_ssc_metric()
        dataset_splits = prepare_splits(
            dataset_dict= _spider_ssc_dataset_dict(),
            add_serialized_schema=_spider_add_serialized_schema,
            pre_process_function=_spider_pre_process_function,
            **_prepare_splits_kwargs,
        )
    elif data_args.dataset == "spider_ssc_sparql":
        # spider4ssc_sparql
        metric = _spider_ssc_sparql_metric()
        dataset_splits = prepare_splits(
            dataset_dict= _spider_ssc_sparql_dataset_dict(),
            add_serialized_schema=_spider_add_serialized_schema,
            pre_process_function=_spider_pre_process_function,
            **_prepare_splits_kwargs,
        )
    elif data_args.dataset == "spider_ssc_cypher":
        # spider4ssc_cypher
        metric = _spider_ssc_cypher_metric()
        dataset_splits = prepare_splits(
            dataset_dict= _spider_ssc_cypher_dataset_dict(),
            add_serialized_schema=_spider_add_serialized_schema,
            pre_process_function=_spider_pre_process_function,
            **_prepare_splits_kwargs,
        )
    elif data_args.dataset == "spider_ssc_joint":
        # spider4ssc_joint
        metric = _spider_ssc_joint_metric()
        dataset_splits = prepare_splits(
            dataset_dict= _spider_ssc_joint_dataset_dict(),
            add_serialized_schema=_spider_add_serialized_schema,
            pre_process_function=_spider_pre_process_function,
            **_prepare_splits_kwargs,
        )
    elif data_args.dataset == "sm3_sql":
        metric = _sm3_sql_metric()
        dataset_splits = prepare_splits(
            dataset_dict= _sm3_sql_dataset_dict(),
            add_serialized_schema=_spider_add_serialized_schema,
            pre_process_function=_spider_pre_process_function,
            **_prepare_splits_kwargs,
        )
    elif data_args.dataset == "sm3_sparql":
        metric = _sm3_sparql_metric()
        dataset_splits = prepare_splits(
            dataset_dict= _sm3_sparql_dataset_dict(),
            add_serialized_schema=_spider_add_serialized_schema,
            pre_process_function=_spider_pre_process_function,
            **_prepare_splits_kwargs,
        )
    elif data_args.dataset == "sm3_cypher":
        metric = _sm3_cypher_metric()
        dataset_splits = prepare_splits(
            dataset_dict= _sm3_cypher_dataset_dict(),
            add_serialized_schema=_spider_add_serialized_schema,
            pre_process_function=_spider_pre_process_function,
            **_prepare_splits_kwargs,
        )
