...
if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
    try:
        ...
        os.environ["CUDA_VISIBLE_DEVICES"]=f"{cfg.get('devices', '0,1')}"
    except Exception as e:
        raise ValueError(f"Failed to load configuration from {sys.argv[1]}: {e}")
...
def main() -> None:
    ...
    os.environ["CUDA_VISIBLE_DEVICES"]=f"{model_args.devices}"
    ...
    if 'trainer_state.json' in model_args.model_name_or_path:
        trainer_state = json.load(open(model_args.model_name_or_path, "r"))
        model_args.model_name_or_path = trainer_state["best_model_checkpoint"]
        logger.info(f"Resolve model_name_or_path to {model_args.model_name_or_path}")
    ...
    if model_args.model_name_or_path == "t5-base-from-scratch":
        model = model_cls_wrapper(T5ForConditionalGeneration)(config=config)
    else:
    ...
    if "spider" in data_args.dataset and not "cosql" in data_args.dataset:
        trainer = SpiderTrainer(**trainer_kwargs)
    elif "sm3" in data_args.dataset:
        trainer = SpiderTrainer(**trainer_kwargs) 
